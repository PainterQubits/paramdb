...
class ParamData(ABC, Generic[ChildNameT]):
    ...
    def _init_from_json(self, json_data: Any) -> None:
        ...
        by ``json.loads()`` and originally constructed by ``self._data_to_json()``. By
        default, this method will pass the JSON data to ``self.__init__()``.

        The object will be generated by ``self.__new__()``, but ``self.__init__()`` has
        not been called and ``self._last_updated_frozen`` is set to False. The last
        updated timestamp is handled separately and does not need to be set here.
        """
    ...
    def from_dict(cls, data_dict: dict[str, Any]) -> Self:
        ...
        param_data._init_from_json(data_dict[_DATA_KEY])
